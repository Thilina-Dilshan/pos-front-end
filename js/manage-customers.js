...
btnSave.on('click', () => {
    ...
    xhr.addEventListener('readystatechange', ()=> {
        if (xhr.readyState === 4){
            [txtId, txtName, txtAddress, txtContact, btnSave].forEach(elm => elm.removeAttr('disabled'));
    ...
    [txtId, txtName, txtAddress, txtContact, btnSave].forEach(elm => elm.attr('disabled', 'true'));
...
function getCustomers() {
    ...
    xhr.addEventListener('readystatechange', () => {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                tbodyElm.empty();
                const customerList = JSON.parse(xhr.responseText);   //customer list
